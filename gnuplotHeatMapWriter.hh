...
template <typename T>
void write(BlockReduction3D2D<T>& blockReduction, int iT, const std::vector<T>& valueArea, const plotParam<T> param) {
  detail::genericHeatMapInterface(blockReduction, blockReduction, iT, valueArea, param);
}

template <typename T>
void write(BlockReduction2D2D<T>& blockReduction, int iT, const std::vector<T>& valueArea, const plotParam<T> param) {
...
void writeHeatMapPlotFile(detailParam<T>& param, const std::vector<T>& valueArea) {
  ...
  if (util::nearZero(valueArea[0] - valueArea[1])) {
