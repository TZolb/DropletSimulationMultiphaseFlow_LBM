 ...
 */
template <typename T>
void write(BlockReduction3D2D<T>& blockReduction, int iT, const std::vector<T>& valueArea = std::vector<T>(2, T()),
           const plotParam<T> param = {}); // = {T(), T()}
 ...
 */
template <typename T>
void write(BlockReduction2D2D<T>& blockReduction, int iT, const std::vector<T>& valueArea = std::vector<T>(2, T()),
